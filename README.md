제시해주신 포트폴리오 리드미의 세련된 HTML/Markdown 혼합 구조를 그대로 살려, **ReadSync 백엔드 (유저/보안 파트)**에 맞게 재구성했습니다.

면접관이 지원자의 **담당 파트(소셜 로그인, JWT 보안, 중복 로그인 방지, JPA)**를 명확히 파악할 수 있도록 핵심 로직과 고민의 흔적을 담았습니다. (...)로 표시된 부분(예: DB 종류, 기간 등)만 실제 환경에 맞게 수정해서 바로 적용하시면 됩니다.
