...
<ul>
  ...
  <li><b>개발 기간</b>: (2025.XX.XX ~ 2026.02.XX)</li>
  <li>
    🔗 <b>포트폴리오 GitHub</b>:
...
<ul>
  ...
  <li>OAuth 2.0 프로토콜을 이용한 <b>소셜 로그인 연동</b> (카카오, 네이버, 구글) 및 자동 회원가입 로직 구축</li>
...
<p align="center">
  <b>Spring Security + JWT + OAuth2.0</b>으로 구현한 안전하고 확장성 높은 인증 시스템<br/>
  소셜 간편 로그인 · 상태 비저장 인증 · 중복 로그인 방지 · JPA 유저 관리
</p>

<hr/>

<h2 id="demo">🎥 데모 & 시연 영상</h2>
<p>
  <b>ReadSync의 전체적인 서비스 흐름과 AI/TTS 기능 완성도</b>를 아래 시연 영상을 통해 확인하실 수 있습니다.<br/>
  특히 영상 초반부의 <b>소셜 로그인 및 유저 온보딩(초기 닉네임/장르 설정)</b> 과정이 제가 백엔드에서 전담하여 구현한 인증/인가 및 유저 파트입니다.
</p>
<ul>
  <li>
    ▶️ <b>YouTube 시연 영상 보기</b>: 
    <a href="https://youtu.be/MfN5DW6VUJo?si=bUEgNXEo0SWwK33H" target="_blank" rel="noreferrer">
      https://youtu.be/MfN5DW6VUJo
    </a>
  </li>
</ul>
...
<div align="center">
  <a href="https://youtu.be/MfN5DW6VUJo?si=bUEgNXEo0SWwK33H" target="_blank" rel="noreferrer">
    <img width="80%" alt="ReadSync Demo Video" src="https://img.youtube.com/vi/MfN5DW6VUJo/maxresdefault.jpg" style="border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);" />
  </a>
  <br/>
  <p><i>(위 이미지를 클릭하면 YouTube 시연 영상으로 이동합니다.)</i></p>
</div>
...
<h2 id="features">2) 🚀 주요 특징</h2>
<ul>
  <li>🛡 <b>안전한 API 보호</b>: Custom JwtFilter를 통한 매 요청 토큰 검증 및 권한 인가</li>
  <li>🔑 <b>소셜 간편 로그인</b>: OAuth2 Client를 활용한 플랫폼(카카오, 네이버, 구글) 연동 및 자동 회원가입</li>
...
<h2 id="stack">3) 🧱 개발 환경 & 기술 스택 (요약)</h2>
<ul>
...
<h2 id="structure">4) 📁 프로젝트 구조 (Security & User 중심)</h2>
<pre><code>plaintext
ReadSync-backend/
...
<h2 id="controls">5) 🔄 API & 인증 플로우 (요약)</h2>
<ul>
...
<h2 id="design">7) 🧩 보안 설계 개요 (요약)</h2>
<ul>
  <li><b>인증 파이프라인</b>: <code>UsernamePasswordAuthenticationFilter</code> 이전에 커스텀 <code>JwtFilter</code>를 배치하여 토큰을 먼저 가로채고 검증.</li>
  <li><b>OAuth2 통합</b>: <code>OAuth2UserService</code>를 상속받아 소셜 플랫폼에서 가져온 속성(Attributes)을 내부 <code>User</code> 엔티티로 자동 매핑.</li>
...
<h2 id="collision">8) ⚙️ 중복 로그인 & 토큰 관리 (요약)</h2>
<ul>
...
<h2 id="insights">11) 🌱 느낀점 및 트러블 슈팅 (Trouble Shooting)</h2>
<ul>
